...
src = cv2.imread('Edit Characters/Capture.PNG', 1)
blurred = cv2.GaussianBlur(src, (5, 5), 0)

blurred_float = blurred.astype(np.float32) / 255.0
edgeDetector = cv2.ximgproc.createStructuredEdgeDetection("model.yml")
edges = edgeDetector.detectEdges(blurred_float) * 255.0
cv2.imshow('edge-raw.jpg', edges)

edges_8u = np.asarray(edges, np.uint8)
filterOutSaltPepperNoise(edges_8u)
cv2.imshow('edge.jpg', edges_8u)
...
cv2.imshow('trimap.png', trimap_print)
...
cv2.imshow('foreground.png', foreground)
cv2.imshow('background.png', background)
cv2.imshow('alpha.png', alpha)
